...
export function useEscrowsByRole(options?: UseEscrowsByRoleOptions) {
  ...
  const [roleState, setRoleState] =
    React.useState<GetEscrowsFromIndexerByRoleParams["role"]>("approver");

  // Use the role from props if provided, otherwise use state
  const role = options?.role ?? roleState;
  ...
  return {
    ...
    formattedRangeLabel,
    role,
    setRole: options?.role ? undefined : setRoleState,
    onClearFilters,
    handleSortingChange,
  } as const;
}
