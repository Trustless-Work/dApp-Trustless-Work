...
import { MilestoneDashboardData, MilestoneWithEscrow } from "../@types/dashboard.entity"
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setData({
        ...
        pendingApproval: allMilestones.filter((m) => m.status === "completed" && !m.approved_flag).length,
        approvedNotReleased: allMilestones.filter((m) => m.approved_flag && !m.releaseFlag).length,
...
const getMilestoneStatusCounts = (milestones: MilestoneWithEscrow[]): MilestoneDashboardData["milestoneStatusCounts"] => {
...
const getMilestoneApprovalTrend = (milestones: MilestoneWithEscrow[]): MilestoneDashboardData["milestoneApprovalTrend"] => {
  const approvedMilestones = milestones.filter((m) => m.approved_flag && m.approvedAt)
...
const getMilestonesByStatus = (milestones: MilestoneWithEscrow[]) => {
  return {
    ...
    completed: milestones.filter((m) => m.status === "completed" && !m.approved_flag).slice(0, 5),
