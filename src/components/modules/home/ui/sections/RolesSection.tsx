...
export const RolesSection = () => {
	...
	const roles = [
		...
		},
		{
			...
			examples: t("home.roles.serviceProvider.examples", { returnObjects: true }),
		...
		},
		{
			...
			examples: t("home.roles.disputeResolver.examples", { returnObjects: true }),
		...
		},
		{
			...
			examples: t("home.roles.platformAddress.examples", { returnObjects: true }),
							...
							<Card className="h-full border-2 border-border/50 bg-card/50 backdrop-blur-sm hover:border-primary/30 transition-all duration-300 hover:shadow-lg hover:shadow-primary/10">
								<CardHeader className="pb-4">
									<div className="flex items-center gap-3 mb-3">
										<div className={`p-3 rounded-xl ${role.color} group-hover:scale-110 transition-transform duration-300`}>
											{role.icon}
										</div>
								...
								<CardContent className="pt-0">
									<div className="space-y-3">
										<div className="flex items-center gap-2 mb-3">
											...
											<span className="text-sm font-medium text-foreground">{t("home.roles.examples.title")}</span>
										</div>

										<div className="space-y-2">
											{Array.isArray(role.examples) ? (
												role.examples.map((example: string | object, exampleIndex: number) => (
													<div key={exampleIndex} className="flex items-start gap-2">
														<div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${role.color.replace("bg-", "bg-").replace("/10", "/30")}`} />
					...
					<Card className="border-2 border-border/50 bg-card/50 backdrop-blur-sm max-w-2xl mx-auto">
						<CardContent className="p-6">
							<div className="flex items-center justify-center gap-3 mb-4">
								...
								<h3 className="text-xl font-semibold">{t("home.roles.cta.title")}</h3>
							</div>
							<p className="text-muted-foreground mb-4">{t("home.roles.cta.description")}</p>
							<a href="https://docs.trustlesswork.com/trustless-work/technology-overview/roles-in-trustless-work" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-primary hover:underline font-medium">
